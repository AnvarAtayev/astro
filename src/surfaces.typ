...
#let circle_band(y1, y2, fill: none, stroke: none) = {
...
#let mercury_crater_edge = rgb(44, 42, 41)
#let mercury_crater = rgb(55, 54, 54)
#let mercury_caloris = rgb(80, 76, 72)
#let venus1 = rgb(100, 100, 85)
#let venus2 = rgb(100, 87, 60)
#let venus3 = rgb(96, 87, 0)
#let uranus_gray = rgb(69, 77, 80)
#let uranus_blue = rgb(69, 93, 90)
...
#let neptune_white = rgb(94, 100, 100)
#let neptune_dark_spot = rgb(10, 22, 58)
#let pluto1 = rgb(82, 77, 70)
#let pluto2 = rgb(24, 18, 100)
#let pluto_heart = rgb(91, 88, 83)
...
#let mercury_surface() = {
  ...
  circle((-0.25, 0.38), radius: 0.32, fill: mercury_caloris, stroke: mercury_crater_edge + 1pt)
  circle((-0.25, 0.38), radius: 0.22, fill: color.mix((mercury_caloris, 55%), (gray, 45%)), stroke: none)
  // craters distributed across the non-Caloris hemisphere
  let c(pos, r) = circle(pos, radius: r, fill: mercury_crater, stroke: mercury_crater_edge + 0.5pt)
  c((0.5, 0.55), 0.09)
  c((0.22, 0.3), 0.075)
  c((0.6, 0.65), 0.04)
  c((0.75, 0.42), 0.03)
...
#let venus_surface() = {
  ...
  let band(y1, y2, c) = circle_band(y1, y2, fill: c)
...
#let earth_surface() = {
  ...
  // polar ice caps
  circle_band(0.84, 1.0, fill: ice)
  circle_band(-1.0, -0.88, fill: ice)
}
...
#let mars_surface() = {
  ...
  // polar ice caps
  circle_band(0.83, 1.0, fill: white.darken(5%))
  circle_band(-1.0, -0.86, fill: white.darken(8%))
}
...
#let jupiter_surface() = {
  ...
  circle_band(0.70, 1.0, fill: b(68%))          // North Polar Region
  circle_band(0.56, 0.68, fill: b(42%))          // North Temperate Zone
  circle_band(0.46, 0.56, fill: b(62%))          // North Temperate Belt
  circle_band(0.10, 0.46, fill: belt)            // North Equatorial Belt
  circle_band(-0.04, 0.10, fill: zone)           // Equatorial Zone
  circle_band(-0.40, -0.04, fill: belt)          // South Equatorial Belt (contains GRS)
  circle_band(-0.52, -0.40, fill: b(42%))        // South Tropical Zone
  circle_band(-0.62, -0.52, fill: b(62%))        // South Temperate Belt
  circle_band(-1.0, -0.70, fill: b(68%))         // South Polar Region
...
#let saturn_surface() = {
  ...
  circle_band(0.78, 1.0, fill: pole)
  circle_band(-1.0, -0.78, fill: pole)
  circle_band(0.56, 0.63, fill: belt)
  circle_band(0.28, 0.37, fill: belt)
  circle_band(0.02, 0.13, fill: belt)
  circle_band(-0.14, -0.04, fill: belt)
  circle_band(-0.38, -0.29, fill: belt)
  circle_band(-0.63, -0.56, fill: belt)
}

#let uranus_surface() = {
  circle((0, 0), radius: 1, fill: uranus_blue, stroke: uranus_gray + 2pt)
  // faint polar collar — slightly brighter near the poles
  let collar = color.mix((uranus_blue, 30%), (white, 70%))
  circle_band(0.72, 1.0, fill: collar)
  circle_band(-1.0, -0.72, fill: collar)
  // very subtle equatorial darkening band
  circle_band(-0.12, 0.12, fill: color.mix((uranus_blue, 80%), (uranus_gray, 20%)))
}
...
#let neptune_surface() = {
  ...
  circle((0.15, -0.22), radius: (0.22, 0.14), fill: neptune_dark_spot, stroke: none)
  circle((0.15, -0.22), radius: (0.16, 0.09), fill: color.mix((neptune_dark_spot, 70%), (neptune2, 30%)), stroke: none)
  // Scooter — bright cirrus-like cloud feature
  circle((-0.3, -0.08), radius: (0.09, 0.04), fill: neptune_white, stroke: none)
}
...
#let pluto_surface() = {
  ...
  circle((-0.02, -0.32), radius: (0.2, 0.22), fill: pluto_heart, stroke: none)  // Sputnik Planitia (left lobe)
  circle((0.2, -0.26), radius: (0.15, 0.17), fill: pluto_heart, stroke: none)   // eastern lobe
  // dark equatorial terrain (Cthulhu Macula)
  circle((-0.45, 0.1), radius: (0.32, 0.12), fill: color.mix((pluto2, 60%), (black, 40%)), stroke: none)
}

#let generic_surface(color: blue) = {
  circle((0, 0), radius: 1, fill: color, stroke: color.lighten(50%) + 2pt)
}

#let surfaces = (
  "sun": sun_surface,
  "mercury": mercury_surface,
  "venus": venus_surface,
  "earth": earth_surface,
  "moon": moon_surface,
  "mars": mars_surface,
  "jupiter": jupiter_surface,
  "saturn": saturn_surface,
  "uranus": uranus_surface,
  "neptune": neptune_surface,
  "pluto": pluto_surface,
)
